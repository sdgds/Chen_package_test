...
**数据流向**：
1. 原始BMTK数据经过转换器生成PKL格式数据
2. PKL数据驱动Chen神经元仿真和网络仿真
3. BMTK数据直接支持NEST高精度仿真
4. 所有仿真结果汇集到可视化分析工具

**调用关系**：
- `interactive_test.py`继承并扩展`test_simulation.py`的功能
- `test_NEST_neuron.ipynb`通过模型映射与`neuron_response_analysis.py`形成对比
- `test_visualization.ipynb`集成展示所有模块的仿真结果
