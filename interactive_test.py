...
def interactive_menu():
    ...
    交互式菜单
    """
    parser = argparse.ArgumentParser(description='V1模型交互式仿真测试')
    
    # 基本参数
    parser.add_argument('--neurons', type=int, default=5000, 
                       help='神经元数量 (默认: 5000)')
    parser.add_argument('--duration', type=int, default=1000,
                       help='仿真时长(ms) (默认: 1000)')
    parser.add_argument('--dt', type=float, default=1.0,
                       help='时间步长(ms) (默认: 1.0)')
    
    # 选择参数
    parser.add_argument('--layer', type=str, choices=['L1', 'L2', 'L3', 'L4', 'L5', 'L6'],
                       help='选择特定层')
    parser.add_argument('--cell-type', type=str, choices=['e', 'i'],
                       help='选择细胞类型 (e=兴奋性, i=抑制性)')
    parser.add_argument('--neuron-ids', type=int, nargs='+',
                       help='指定神经元ID列表')
    
    # 输出参数
    parser.add_argument('--save-plot', type=str,
                       help='保存图形的文件路径')
    parser.add_argument('--export-neuron', type=int,
                       help='导出指定神经元的数据')
...
def main():
    ...
    # 运行仿真
    print("\n正在运行仿真...")
    simulation_results = tester.run_simulation(cell, lgn_input)
    ...
    # 打印分析结果
    print("\n分析结果:")
    print(f"  神经元数量: {analysis['n_neurons']}")
    print(f"  平均发放率: {analysis['mean_firing_rate']:.2f} Hz")
    print(f"  发放率标准差: {analysis['std_firing_rate']:.2f} Hz")
    print(f"  平均变异系数: {analysis['mean_cv']:.2f}")
    print(f"  同步性指数: {analysis['synchrony_index']:.3f}")
    print(f"  膜电位范围: [{analysis['voltage_stats']['min']:.1f}, {analysis['voltage_stats']['max']:.1f}] mV")
    
    # 绘制详细活动图
    print("\n生成活动图...")
    tester.plot_detailed_activity(
        simulation_results, 
        selected_indices, 
        analysis,
    ...
    # 导出神经元数据
    if args.export_neuron is not None and args.export_file:
        print(f"\n导出神经元 {args.export_neuron} 的数据...")
        tester.export_neuron_data(
            simulation_results,
            args.export_neuron,
            args.export_file
        )
